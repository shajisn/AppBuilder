 ...
 */
public class CrudGridRecyclerAdapter extends RecyclerView.Adapter<CrudGridRecyclerAdapter.ViewHolder> {

    private ArrayList<User> items = new ArrayList<>();
    private SignUpResponse mSignUpResponse;
    private Context mContext;
    private Set<User> selectedTasks = new HashSet<>();
    private static final EventBus eventBus = EventBus.getDefault();


    public CrudGridRecyclerAdapter(ArrayList<User> items, SignUpResponse signUpResponse) {
        this.items = items;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View contactView = inflater.inflate(R.layout.single_grid_item, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        for (Field field : mSignUpResponse.getFields()) {
            if (field.getId().equalsIgnoreCase(Constants.fieldTypeName))
            holder.name.setText(Util.identifyTheTypeOfFieldToExtractTheData(field,user));
        }
        Glide.with(mContext)
                .load(user.getImage())
                .centerCrop()
